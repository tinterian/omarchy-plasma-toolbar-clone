// ============================================================================
// FILE:    ~/.config/omarchy/plugins/spencer.menu/Menu.qml
// TASK:    Right-click context menu (Open / Pin to bar / Pin to dock / Uninstall) for app rows in the Omarchy menu
// STATUS:  DONE
// OWNER:   Claude Code 2026-09-20 13:40
//
// WHAT THIS FILE DOES:
//   Clone of the built-in omarchy.menu (clonedFrom omarchy.menu) plus a right-click popup
//   on app rows: contextMenuOpen/contextOverlay, openContextMenu/runContextAction. Uninstall
//   reuses the existing delete-confirm dialog. Pin to/Unpin from bar and dock call bin/pin-app.sh;
//   pin state comes from shell.json (pinsFile / loadPins) so the popup offers Pin or Unpin per app.
//
// NEXT STEP (if not DONE):
//   VERIFY: a real mouse right-click on an app row (only the Menu-key route was exercised, via wtype;
//   both call openContextMenu). Also uninstall was never run. manifest.json has keepLoaded=false and
//   appLibrary falls back to LocalAppLibrary.qml: the host destroys a kept-loaded third-party menu's
//   shell API ~1s after startup, leaving shell.appLibrary null.
// ============================================================================
import Quickshell
...
Item {
  ...
  // Falls back to a local implementation when the host gives this third-party
  // plugin a null appLibrary (see LocalAppLibrary.qml for why).
  readonly property var appLibrary: (root.shell && root.shell.appLibrary) ? root.shell.appLibrary : localAppLibrary
  LocalAppLibrary { id: localAppLibrary; omarchyPath: root.omarchyPath }
  property bool deleteConfirmOpen: false
  property var deleteTarget: null
  // Right-click menu for app rows: Open / Uninstall. Position is in card
  // coordinates; contextIndex is the keyboard/hover-highlighted action.
  ...
  // Pin state, read live from shell.json so the popup offers Pin or Unpin per app.
  property var barPinned: ({})
  property var dockPinned: ({})
  property bool barAvailable: false
  property bool dockAvailable: false
  readonly property string pinScript: Qt.resolvedUrl("bin/pin-app.sh").toString().replace("file://", "")

  function pinKey(id) {
    return String(id || "").toLowerCase().replace(/\.desktop$/, "")
  }

  function loadPins(rawText) {
    var bar = ({}), dock = ({})
    var hasBar = false, hasDock = false
    try {
      ...
      if (layout && typeof layout === "object") {
        ...
        for (var region in layout) {
          ...
          for (var i = 0; i < list.length; i++)
            if (list[i] && list[i].app) bar[root.pinKey(list[i].app)] = true
        }
      }
      var plugins = (cfg && cfg.plugins) || []
      for (var j = 0; j < plugins.length; j++) {
        if (!plugins[j] || plugins[j].id !== "animated.dock") continue
        hasDock = true
        var items = plugins[j].items || []
        for (var k = 0; k < items.length; k++)
          if (items[k] && items[k].desktop) dock[root.pinKey(items[k].desktop)] = true
      }
    } catch (e) { }
    root.barPinned = bar
    root.dockPinned = dock
    root.barAvailable = hasBar
    root.dockAvailable = hasDock
  }

  function buildContextActions(appId) {
    var key = root.pinKey(appId)
    var out = [{ id: "open", label: "Open" }]
    if (root.barAvailable)
      out.push(root.barPinned[key] ? { id: "bar-unpin", label: "Unpin from bar" } : { id: "bar-pin", label: "Pin to bar" })
    if (root.dockAvailable)
      out.push(root.dockPinned[key] ? { id: "dock-unpin", label: "Unpin from dock" } : { id: "dock-pin", label: "Pin to dock" })
    out.push({ id: "uninstall", label: "Uninstall" })
    return out
  }
  ...
  FileView {
    ...
    // text() still holds the old content inside fileChanged; reload first and read it in onLoaded.
    onFileChanged: pinsFile.reload()
  }
  onOpenedChanged: if (!opened) { deleteConfirmOpen = false; deleteTarget = null; contextMenuOpen = false; contextTarget = null }
  ...
  function openContextMenu(row, x, y) {
    if (root.dmenuActive || root.deleteConfirmOpen || !row || row.kind !== "app") return
    root.cursorActive = true
    root.selectedIndex = row.index
    root.contextTarget = { appId: row.appId, label: row.label }
    root.contextActions = root.buildContextActions(row.appId)
    root.contextX = x
    root.contextY = y
    root.contextIndex = 0
  ...
  // Keyboard route to the same popup (Menu key): anchors under the selected row.
  function openContextMenuForSelected() {
    ...
    var item = resultList.itemAtIndex(root.selectedIndex)
    if (!item) return
    var p = item.mapToItem(contextOverlay, item.width / 2, item.height)
  ...
  function runContextAction(actionId) {
    ...
    if (actionId === "open") {
      ...
      opened = false
      filterText = ""
      if (root.appLibrary) root.appLibrary.launch(target.appId, target.label)
    } else if (actionId === "bar-pin" || actionId === "bar-unpin" || actionId === "dock-pin" || actionId === "dock-unpin") {
      // shell.json is replaced by rename, which the FileView watcher above does not follow,
      // so update the local pin state right away instead of waiting for a file event.
      var pinKeyName = root.pinKey(target.appId)
      var forBar = actionId.indexOf("bar-") === 0
      var nextPins = ({})
      var currentPins = forBar ? root.barPinned : root.dockPinned
      for (var existing in currentPins) nextPins[existing] = true
      if (actionId.slice(-4) === "-pin") nextPins[pinKeyName] = true
      else delete nextPins[pinKeyName]
      if (forBar) root.barPinned = nextPins
      else root.dockPinned = nextPins
      Util.execDetached(Util.shellQuote(root.pinScript) + " " + actionId + " " + Util.shellQuote(target.appId) + " " + Util.shellQuote(target.label))
      Qt.callLater(function() { keyCatcher.forceActiveFocus() })
    } else if (actionId === "uninstall") {
      // Same confirm dialog the Delete key uses.
      root.deleteTarget = target
  ...
  function activateIndex(index, fromPointer) {
    if (root.deleteConfirmOpen || root.contextMenuOpen) return
    if (root.dmenuActive) {
      if (root.mode === "input") {
  ...
  PanelWindow {
    ...
    BorderSurface {
      ...
      Item {
        ...
        Keys.onPressed: function(event) {
          ...
          if (root.contextMenuOpen) {
            ...
            if (event.key === Qt.Key_Escape || event.key === Qt.Key_Left) root.closeContextMenu()
            else if (event.key === Qt.Key_Up) root.contextIndex = (root.contextIndex + n - 1) % n
            else if (event.key === Qt.Key_Down) root.contextIndex = (root.contextIndex + 1) % n
            else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter || event.key === Qt.Key_Right)
              root.runContextAction(root.contextActions[root.contextIndex].id)
            event.accepted = true
            return
          }
      ...
      // Right-click popup for app rows. Sits above the list (z 30); a click
      ...
      Item {
        ...
        Rectangle {
          id: contextPopup
          readonly property real itemHeight: Style.space(30)
          readonly property real inset: Style.space(4)
          width: Style.space(180)
          height: root.contextActions.length * itemHeight + inset * 2
          x: Math.max(inset, Math.min(root.contextX, contextOverlay.width - width - inset))
          y: Math.max(inset, Math.min(root.contextY, contextOverlay.height - height - inset))
          radius: root.cornerRadius
          // Opaque copy of the menu background so rows underneath don't show through.
          ...
          Column {
            anchors.fill: parent
            anchors.margins: contextPopup.inset
            ...
            Repeater {
              ...
              delegate: Rectangle {
                ...
                width: parent.width
                height: contextPopup.itemHeight
                radius: root.cornerRadius
                color: root.contextIndex === index ? root.selectedBackground : "transparent"
                ...
                Text {
                  ...
                  color: root.contextIndex === index ? root.selectedText : root.foreground
                ...
                MouseArea {
                  ...
                  onEntered: root.contextIndex = index
              ...
              MouseArea {
                ...
                onClicked: function(mouse) {
                  if (mouse.button === Qt.RightButton) {
                    ...
                    var p = mouseArea.mapToItem(contextOverlay, mouse.x, mouse.y)
