...
Item {
  ...
  // Not `required`: the third-party plugin Loader that hosts a cloned bar
  // constructs this Item first and assigns properties afterward (via
  // shell.configureBar in onLoaded), which a `required property` rejects at
  // construction time — that failure is what actually blanks the bar.
  ...
  // Floating-panel look: gap between the bar and the screen edges, corner
  // rounding for the panel box, and a translucent fill (real blur comes from
  // the omarchy-bar layer rule in hypr/looknfeel.lua). Kept independent of
  // Style.cornerRadius, which mirrors Hyprland's window rounding and can be 0.
  // Raw pixels (not run through Style.space's spacing-scale rounding), set to
  // match the window's own natural gap to the screen edge - gaps_out (10) +
  // border_size (2) - so the gap above the bar reads the same as the gaps on
  // the window's other three edges.
  property int floatGap: 12
  property int floatRadius: 14
  // The alpha the panel sits at normally. Kept separate from the double-click
  // "see-through, matches wallpaper" toggle (transparent/panelAlpha below),
  // which stays a fixed, near-invisible value regardless of this setting.
  property real opacityLevel: 0.4
  ...
  // rewrite the Lua config, so a `hyprctl reload` (or reboot) reverts to that
  // file's value until this shell restarts and re-applies the saved one.
  ...
  function persistFloatingSettings() {
    floatingSettingsFile.setText(JSON.stringify({
      gap: root.floatGap,
  ...
  onBlurSizeChanged: applyBlurSize(blurSize)

  FileView {
    id: floatingSettingsFile
    path: root.floatingSettingsPath
    watchChanges: true
    atomicWrites: true
    printErrors: false
  ...
  // panelBackground is a computed (readonly) property, not a plain assignable
  // one, so it can't take its own Behavior — QML rejects that at load time.
  // It re-derives every frame from `background`, so animating `background`
  // already carries panelBackground along smoothly.
  ...
  component BarPanel: PanelWindow {
    ...
    // Only the actually-anchored edges (mirrors the `anchors` block below)
    // get the floating gap. The far, non-anchored edge must stay 0: Quickshell's
    // Auto exclusion zone counts it as reserved space too, which was stacking
    // with Hyprland's own gaps_out and doubling the gap on that side.
    margins {
      top: root.barHidden && root.position === "top" ? -root.barSize : (root.position === "top" || root.vertical ? root.floatGap : 0)
      bottom: root.barHidden && root.position === "bottom" ? -root.barSize : (root.position === "bottom" || root.vertical ? root.floatGap : 0)
      left: root.barHidden && root.position === "left" ? -root.barSize : (root.position === "left" || !root.vertical ? root.floatGap : 0)
      right: root.barHidden && root.position === "right" ? -root.barSize : (root.position === "right" || !root.vertical ? root.floatGap : 0)
    }
    ...
    // The window itself stays fully transparent; the rounded BorderSurface
    // below paints the actual floating, translucent panel box.
    color: "transparent"
  ...
  component CenterGestureArea: MouseArea {
    ...
    // rather than just clearing its own `open` (which would sever the binding
    // to settingsOpen below and leave this stuck open on the next toggle).
    function close() { gestureArea.settingsOpen = false }
    ...
    onClicked: function(mouse) {
      ...
      if (mouse.button === Qt.RightButton) {
        gestureArea.settingsOpen = !gestureArea.settingsOpen
        mouse.accepted = true
      }
    }
    ...
    PopupCard {
      id: floatingSettingsPopup
      anchorItem: gestureArea
      bar: root
      owner: gestureArea
      open: gestureArea.settingsOpen
      centerOnBar: true
      contentWidth: floatingSettingsPopup.fittedContentWidth(Style.space(260))
      contentHeight: floatingSettingsPopup.fittedContentHeight(settingsColumn.implicitHeight)
